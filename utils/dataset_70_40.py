    ...
    for filename in sorted(os.listdir(_data_path)):
        ...
        strings_to_drop = ['city', 'latest', 'Latest', 'First_date', 'First date', 'start_date', 'ДАТА', 'upper_b', 'Unnamed', 'Знач', 'FIO',
                          #'SQM_SINGLE_MATS_IN_ACTIVE_SPECIFICATIONS',
                           'sum_recalculations', 'er_avg_1', 'er_avg_0', 'winter_sum_1', 'winter',
                           'undelivered', 'Entity', 'Founder', 'kbk', 'code', # 'Tax', 'tax', 'Fines', 'Arrears', 'Penalti',
                           #'weather',
                           #'address',
...
def add_quality_features(df: pd.DataFrame, _total_ds: pd.DataFrame):
    ...
    # v9:
    df['spacetime_area_mean'] = df['total_spacetime_area'] / df['Active_months']
    df['total_spacetime_area_fraction'] = df['total_spacetime_area'] * 100 / df['total_spacetime_area'].sum()

    df['Turnover_mean_last_12'] = df['Turnover_sum_last_12'] / df['Active_months']
    df['sqm_mean'] = df['sqm_sum'] / df['Active_months']
    df['Frequency_of_changes_mean'] = df['Frequency_of_changes_sum'] / df['Active_months']
    df = df.drop(columns=['Frequency_of_changes_sum'])
    df['n_recalculations_mean'] = df['total_recalculations'] / df['Active_months']
    df = df.replace([np.nan, pd.NA], 0)

    # lets try calculate ball derivative:
    for col in df.columns:
        if 'Credit' in col and 'Надежность' not in col and ('_before' in col or '_after' in col):
            # Get the base metric name by removing _before/_after
            metric = col.replace("_before", "").replace("_after", "")
            # Check if both before and after columns exist
            if f"{metric}_before" in df.columns and f"{metric}_after" in df.columns:
                # Create derivative column
                print("create")
                df[f"{metric}_deriv"] = 0  # Initialize with default value

                # Apply your conditions
                df.loc[
                    (df[f"{metric}_before"] == df[f"{metric}_after"]),
                    f"{metric}_deriv"
                ] = 0

                df.loc[
                    (df[f"{metric}_before"] == 'Низкий балл') &
                    (df[f"{metric}_after"] == 'Средний балл'),
                    f"{metric}_deriv"
                ] = 1

                df.loc[
                    (df[f"{metric}_before"] == 'Низкий балл') &
                    (df[f"{metric}_after"] == 'Высокий балл'),
                    f"{metric}_deriv"
                ] = 2

                df.loc[
                    (df[f"{metric}_before"] == 'Средний балл') &
                    (df[f"{metric}_after"] == 'Низкий балл'),
                    f"{metric}_deriv"
                ] = -1

                df.loc[
                    (df[f"{metric}_before"] == 'Высокий балл') &
                    (df[f"{metric}_after"] == 'Низкий балл'),
                    f"{metric}_deriv"
                ] = -2
                print(df.columns)
                print(metric)
                df = df.drop(columns=metric+'_before')

#    df['DRIVER_FIO'] = df['DRIVER_FIO'].where(df['DRIVER_FIO'].map(df['DRIVER_FIO'].value_counts()) > 10, 'Other').astype('str')
    #df['tenure_adjusted_activity'] = df['Dur_months'] / df['Active_months']
