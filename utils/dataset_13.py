...
def add_quality_features(df: pd.DataFrame, _total_ds: pd.DataFrame):
    ...
    for col in df.columns:
        if ('Credit' in col or 'bankrot' in col) and 'Надежность' not in col and ('_before' in col or '_after' in col):
            ...
            if f"{metric}_before" in df.columns and f"{metric}_after" in df.columns:
                ...
                df = df.drop(columns=metric+'_after')
...
def create_features_for_datasets(_datasets: list, _config):
    improved_datasets = []
